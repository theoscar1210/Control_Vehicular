...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { background: #5B8238; color: white; padding: 15px; margin-bottom: 20px; }
        .header-content { display: table; width: 100%; }
        .header-logo { display: table-cell; width: 80px; vertical-align: middle; }
        .header-logo img { width: 60px; height: auto; }
        .header-title { display: table-cell; vertical-align: middle; padding-left: 15px; }
        .header h1 { font-size: 18px; margin-bottom: 5px; }
        .header .placa { background: white; color: #333; padding: 8px 15px; display: inline-block; font-size: 20px; font-weight: bold; letter-spacing: 2px; }
        .section { margin-bottom: 15px; }
        .section-title { background: #f8f9fa; padding: 8px 10px; font-weight: bold; border-left: 4px solid #5B8238; margin-bottom: 10px; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 15px; }
        th, td { padding: 8px; text-align: left; border-bottom: 1px solid #ddd; }
        th { background: #f8f9fa; font-weight: bold; width: 35%; }
        .estado-vigente { color: #28a745; font-weight: bold; }
        .estado-por_vencer { color: #ffc107; font-weight: bold; }
        .estado-vencido { color: #dc3545; font-weight: bold; }
        .estado-sin_registro { color: #6c757d; }
        .doc-card { border: 1px solid #ddd; padding: 10px; margin-bottom: 10px; }
        .doc-card h4 { margin-bottom: 5px; }
...
<body>
        ...
        <table style="border: none; width: 100%;">
            <tr>
                ...
                <td style="border: none; width: 50%;">
                    <h1>FICHA DE VEHÍCULO <span class="confidencial-badge">CONFIDENCIAL</span></h1>
    ...
    <div class="section">
        ...
        <table>
            ...
            <tr><th>Estado</th><td>{{ $vehiculo->estado }}</td></tr>
            <tr><th>Fecha Registro</th><td>{{ \Carbon\Carbon::parse($vehiculo->fecha_registro)->format('d/m/Y') }}</td></tr>
        </table>
    </div>
    ...
    <div class="two-col">
        <div class="section">
            ...
            <table>
                <tr><th>Nombre</th><td>{{ $vehiculo->propietario->nombre }} {{ $vehiculo->propietario->apellido }}</td></tr>
                <tr><th>Documento</th><td>{{ $vehiculo->propietario->tipo_doc }} {{ $vehiculo->propietario->identificacion }}</td></tr>
            </table>
            @else
    ...
    <div class="two-col">
        <div class="section">
            ...
            <table>
                <tr><th>Nombre</th><td>{{ $vehiculo->conductor->nombre }} {{ $vehiculo->conductor->apellido }}</td></tr>
                <tr><th>Documento</th><td>{{ $vehiculo->conductor->tipo_doc }} {{ $vehiculo->conductor->identificacion }}</td></tr>
                <tr><th>Teléfono</th><td>{{ $vehiculo->conductor->telefono ?? 'N/A' }}</td></tr>
            </table>
            @else
